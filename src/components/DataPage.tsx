...
export default function DataPage({ locale }: { locale: Locale }) {
  ...
  return (
    <main className="data-page">
      ...
      <section className="source-registry"><header><h2>{locale === 'ko' ? '출처 레지스트리' : 'Source registry'}</h2><span>{isLoading ? 'Loading…' : error ? 'Unavailable' : `${data?.generatedAt.slice(0, 10)} snapshot`}</span></header><div className="source-table" role="table"><div className="source-row source-head" role="row"><span>{locale === 'ko' ? '기관·자료' : 'Provider & dataset'}</span><span>{locale === 'ko' ? '적용 기간' : 'Coverage'}</span><span>{locale === 'ko' ? '갱신' : 'Cadence'}</span><span>{locale === 'ko' ? '원문' : 'Source'}</span></div>{data?.sources.map((source) => <div className="source-row" role="row" key={source.id}><span><b>{source.provider}</b><small>{source.title}</small></span><span>{source.coveragePeriod}</span><span>{source.cadence}</span><a href={source.url} target="_blank" rel="noreferrer"><ExternalLink size={14} />{locale === 'ko' ? '열기' : 'Open'}</a></div>)}</div>{error ? <button className="retry-button" onClick={() => void refetch()}>{locale === 'ko' ? '데이터 다시 불러오기' : 'Retry data load'}</button> : null}</section>
    </main>
  )
}
