  ...
  return (
    <main className="data-page">
      ...
      <section className="source-registry">
        <header><h2>{locale === 'ko' ? '출처 레지스트리' : 'Source registry'}</h2><span>{isLoading ? (locale === 'ko' ? '불러오는 중…' : 'Loading…') : error ? (locale === 'ko' ? '사용할 수 없음' : 'Unavailable') : `${data?.generatedAt.slice(0, 10)} snapshot`}</span></header>
        {error ? <div className="data-error" role="alert"><p>{locale === 'ko' ? '출처 데이터를 불러오지 못했습니다.' : 'Source data could not be loaded.'}</p><button onClick={() => void refetch()}>{locale === 'ko' ? '다시 시도' : 'Try again'}</button></div> : null}
        <div className="source-table" role="table" aria-label={locale === 'ko' ? '공식 출처' : 'Official sources'}>
          <div className="source-row source-head" role="row"><span role="columnheader">{locale === 'ko' ? '기관·자료' : 'Provider & dataset'}</span><span role="columnheader">{locale === 'ko' ? '적용 기간' : 'Coverage'}</span><span role="columnheader">{locale === 'ko' ? '갱신' : 'Cadence'}</span><span role="columnheader">{locale === 'ko' ? '원문' : 'Source'}</span></div>
          {data?.sources.map((source) => <div className="source-row" role="row" key={source.id}><span role="cell"><b>{source.provider}</b><small>{source.title}</small></span><span role="cell">{source.coveragePeriod}</span><span role="cell">{source.cadence}</span><span role="cell"><a href={source.url} target="_blank" rel="noreferrer"><ExternalLink size={14} />{locale === 'ko' ? '열기' : 'Open'}</a></span></div>)}
        </div>
      </section>
    </main>
  )
