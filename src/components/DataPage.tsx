...
export default function DataPage({ locale }: { locale: Locale }) {
  ...
  return (
    <main id="main-content" tabIndex={-1} className="data-page">
      ...
      <section className="source-registry"><header><h2 id="source-registry-title">{locale === 'ko' ? '출처 레지스트리' : 'Source registry'}</h2><span>{isLoading ? 'Loading…' : error ? 'Unavailable' : `${data?.generatedAt.slice(0, 10)} snapshot`}</span></header><div className="source-table" role="table" aria-labelledby="source-registry-title"><div className="source-row source-head" role="row"><span role="columnheader">{locale === 'ko' ? '기관·자료' : 'Provider & dataset'}</span><span role="columnheader">{locale === 'ko' ? '적용 기간' : 'Coverage'}</span><span role="columnheader">{locale === 'ko' ? '갱신' : 'Cadence'}</span><span role="columnheader">{locale === 'ko' ? '원문' : 'Source'}</span></div>{data?.sources.map((source) => <div className="source-row" role="row" key={source.id}><span role="cell"><b>{source.provider}</b><small>{source.title}</small></span><span role="cell">{source.coveragePeriod}</span><span role="cell">{source.cadence}</span><span role="cell"><a href={source.url} target="_blank" rel="noreferrer" aria-label={`${locale === 'ko' ? `${source.provider} 원문 새 탭에서 열기` : `Open ${source.provider} source in a new tab`}`}><ExternalLink size={14} aria-hidden="true" />{locale === 'ko' ? '열기' : 'Open'}</a></span></div>)}</div>{error ? <button className="retry-button" onClick={() => void refetch()}>{locale === 'ko' ? '데이터 다시 불러오기' : 'Retry data load'}</button> : null}</section>
    </main>
  )
}
