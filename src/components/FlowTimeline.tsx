    ...
    <section className="flow-timeline">
      ...
      <div className="timeline-body">
        ...
        <div className="timeline-track" style={{ '--progress': `${step / (events.length - 1) * 100}%` } as React.CSSProperties}>
          ...
          {events.map(([time, ko, en], index) => <button key={time} className={`${index <= step ? 'complete' : ''} ${index === step ? 'current' : ''}`} onClick={() => { setStep(index); setPlaying(false) }}><time>{time}</time><i /><b>{locale === 'ko' ? ko : en}</b><small>{index === step ? (selected === 'chips-fedwire' ? 'CHIPS / Fedwire' : selected.toUpperCase()) : '—'}</small></button>)}
        </div>
