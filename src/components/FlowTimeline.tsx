  ...
  return (
    <section className="flow-timeline">
      ...
      <div className="timeline-body">
        <div className="playback"><span>{t('timeline.play')}</span><div><button type="button" onClick={() => { setPlaying(false); setStep(0) }} aria-label="Restart"><RotateCcw size={15} /></button><button type="button" className="play" onClick={() => setPlaying((value) => !value)} aria-pressed={playing} aria-label={playing ? t('timeline.pause') : t('timeline.play')}>{playing ? <Pause /> : <Play />}</button><button type="button" className="speed-button" onClick={() => setSpeed((value) => value === 1 ? 2 : 1)} aria-pressed={speed === 2} aria-label={`Playback speed ${speed}x`}>{speed}×</button></div><small>{t('notices.simulated')}</small></div>
        <div className="timeline-track" style={{ '--progress': `${step / (EVENTS.length - 1) * 100}%` } as React.CSSProperties}>
          <span className="track-line" />
          {EVENTS.map(([time, ko, en], index) => <button type="button" key={time} className={index <= step ? 'complete' : ''} aria-current={index === step ? 'step' : undefined} onClick={() => setStep(index)}><time>{time}</time><i /><b>{locale === 'ko' ? ko : en}</b><small>{index === step ? (selected === 'chips-fedwire' ? 'CHIPS / Fedwire' : selected.toUpperCase()) : '—'}</small></button>)}
        </div>
