  ...
  return (
    <aside className="network-sidebar" aria-labelledby="network-sidebar-title">
      ...
      <div className="network-search">
        ...
        <input ref={searchRef} type="search" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Escape') setQuery('') }} aria-label={locale === 'ko' ? '금융 네트워크 검색' : 'Search financial networks'} placeholder={locale === 'ko' ? '네트워크·기능 검색' : 'Search network or function'} />
        {query ? <button type="button" onClick={() => { setQuery(''); searchRef.current?.focus() }} aria-label={locale === 'ko' ? '네트워크 검색어 지우기' : 'Clear network search'}><X size={12} /></button> : <kbd aria-hidden="true">/</kbd>}
      </div>
      ...
      {filteredNetworks.length === 0 ? <p className="network-empty" role="status">{locale === 'ko' ? '일치하는 네트워크가 없습니다.' : 'No matching networks.'}</p> : null}
